...
public class CRUDOrderController {
    ...
    public String listOrders(@RequestParam(value = "status", required = false) String status,
        ...
        // Lấy user ADMIN đang login (nếu bạn cần)
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        String adminName = auth.getName(); // username admin đang login
    ...
    public ResponseEntity<?> updateOrderStatus(@PathVariable("orderId") Long orderId,
        ...
        try {
            ...
            // Lấy username của admin đang cập nhật
            Authentication auth = SecurityContextHolder.getContext().getAuthentication();
            ...
            return ResponseEntity.ok(Map.of(
                    "success", true,
                    "message", "Cập nhật trạng thái thành công bởi " + adminName
            ));
